...
exports.vpzNewsSlashCommand = void 0;
exports.handleVpzNewsSlashCommand = handleVpzNewsSlashCommand;
exports.handleVpzNewsModal = handleVpzNewsModal;
const discord_js_1 = require("discord.js");
const config_1 = require("../config");
exports.vpzNewsSlashCommand = new discord_js_1.SlashCommandBuilder()
    .setName("vpz_news")
...
function mkModal(roleId) {
    const modal = new discord_js_1.ModalBuilder().setCustomId(`${MODAL_ID_PREFIX}${roleId}`).setTitle("VPZ News");
    const text = new discord_js_1.TextInputBuilder()
        ...
        .setMaxLength(2000)
        .setRequired(true);
    modal.addComponents(new discord_js_1.ActionRowBuilder().addComponents(text));
    return modal;
}
async function handleVpzNewsSlashCommand(interaction) {
    ...
    await interaction.showModal(mkModal(role.id)).catch(async () => {
        await interaction.reply({ content: "Не удалось открыть окно ввода.", flags: discord_js_1.MessageFlags.Ephemeral }).catch(() => { });
    });
    return true;
}
async function handleVpzNewsModal(interaction) {
    ...
    const caller = await guild.members.fetch(interaction.user.id).catch(() => null);
    if (!caller || !(await isModerator(caller))) {
        await interaction.reply({ content: "Нет прав.", flags: discord_js_1.MessageFlags.Ephemeral }).catch(() => { });
        return true;
    }
    ...
    if (!roleId || !text) {
        await interaction.reply({ content: "Пустой текст/роль.", flags: discord_js_1.MessageFlags.Ephemeral }).catch(() => { });
        return true;
    }
    ...
    await interaction.editReply({
        content: `Рассылка завершена. Роль: <@&${roleId}>\n` +
            `**Доставлено:** ${ok}\n` +
            `**Не доставлено:** ${fail}\n` +
