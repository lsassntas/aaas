...
async function handleInteraction(interaction) {
    try {
        ...
        if (interaction.isModalSubmit()) {
            ...
            {
                const handled = await (0, vpzNews_1.handleVpzNewsModal)(modal);
                if (handled)
                    return;
            }
...
async function handleButton(interaction) {
    ...
    if (customId.startsWith("contract:j:") || customId.startsWith("contract:l:")) {
        const handled = await (0, contractPanel_1.handleContractButton)(interaction);
        if (handled)
            return;
    }
        ...
        if (action === "review") {
            ...
            try {
                ...
                if (appChannel && appChannel.isTextBased() && "permissionOverwrites" in appChannel) {
                    await appChannel.permissionOverwrites
                        .edit(applicantId, {
                        ViewChannel: true,
                        SendMessages: true,
            ...
            try {
                const applicant = await interaction.client.users.fetch(applicantId);
                await applicant.send(`Ваша заявка взята на рассмотрение.\n` +
...
async function handleModalSubmit(interaction) {
    ...
    try {
        const recruiterRoleIds = (config_1.config.RECRUITER_ROLE_IDS?.length ? config_1.config.RECRUITER_ROLE_IDS : config_1.config.MODERATOR_ROLE_IDS) ?? [];
        ...
        const dmText = `Обнаружена новая заявка.\n` +
