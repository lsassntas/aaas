...
import { appendWeeklyTicketEvent } from "../storage/weeklyTickets";
...
export async function handleInteraction(interaction: any) {
  try {
    if (interaction.isChatInputCommand && interaction.isChatInputCommand()) {
      const handledRp = await handleRpSlashCommand(interaction);
      if (handledRp) return;
      const handledDm = await handleRassylkaSlashCommand(interaction);
      if (handledDm) return;
    }
    ...
    if (interaction.isModalSubmit()) {
      ...
      {
        const handled = await handleVpzNewsModal(modal);
        if (handled) return;
      }
    ...
    if (code === 10062 || code === "UND_ERR_CONNECT_TIMEOUT" || anyErr?.name === "ConnectTimeoutError") return;

    console.error(e);
    if (interaction?.isRepliable?.() && !interaction.replied && !interaction.deferred) {
      await interaction.reply({ content: "Ошибка при обработке запроса.", flags: MessageFlags.Ephemeral }).catch(() => {});
    }
  }
}
...
async function handleButton(interaction: ButtonInteraction) {
  ...
  if (customId.startsWith("contract:j:") || customId.startsWith("contract:l:")) {
    const handled = await handleContractButton(interaction);
    if (handled) return;
  }
  ...
  if (customId === PANEL_APPLY_CUSTOM_ID) {
    try {
      ...
      const code = e?.code;
      if (code === 10062 || code === "UND_ERR_CONNECT_TIMEOUT" || e?.name === "ConnectTimeoutError") return;
      throw e;
    }
    return;
  }
  ...
  if (customId.startsWith("family:review:")) {
    ...
    // When moderator takes the application for review:
    // - allow the applicant to write in their application channel
    // - notify the applicant via DM
    if (action === "review") {
      const applicantId = record.discordUserId;
      try {
        const appChannel = await interaction.client.channels.fetch(record.discordChannelId).catch(() => null);
        if (appChannel && appChannel.isTextBased() && "permissionOverwrites" in appChannel) {
          await (appChannel as any).permissionOverwrites
            .edit(applicantId, {
              ViewChannel: true,
              SendMessages: true,
        ...
        console.warn(`Failed to update channel permissions for applicant ${applicantId}:`, e);
      }

      try {
        const applicant = await interaction.client.users.fetch(applicantId);
        await applicant.send(
          `Ваша заявка взята на рассмотрение.\n` +
            `Теперь вы можете писать в канал заявки: <#${record.discordChannelId}>`,
        );
      } catch (e) {
        console.warn(`Failed to DM applicant ${applicantId}:`, e);
      }
    }

    await withFolderLock(folderKey, async () => {
      if (action === "accept" || action === "reject") {
        await appendWeeklyTicketEvent({
...
async function handleModalSubmit(interaction: ModalSubmitInteraction) {
  ...
  // DM recruiters (best-effort)
  try {
    const recruiterRoleIds =
      (config.RECRUITER_ROLE_IDS?.length ? config.RECRUITER_ROLE_IDS : config.MODERATOR_ROLE_IDS) ?? [];

    await guild.members.fetch().catch(() => null);

    const targets = new Map<string, GuildMember>();
    for (const roleId of recruiterRoleIds) {
      const role = await guild.roles.fetch(roleId).catch(() => null);
      if (!role) continue;
      for (const [id, m] of role.members) {
        if (!targets.has(id)) targets.set(id, m);
      }
    }
    ...
    console.warn(`Failed to DM recruiters for application ${applicationId}:`, e);
  }
