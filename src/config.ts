...
export type Config = {
  ...
  /** Рассылка `/rassylka`: сообщение в ЛС только участникам с этой ролью (можно переопределить `BROADCAST_DM_TARGET_ROLE_ID`). */
  BROADCAST_DM_TARGET_ROLE_ID: string;
};
...
export const config: Config = {
  ...
  BROADCAST_DM_TARGET_ROLE_ID:
    parseDiscordIdOptional("BROADCAST_DM_TARGET_ROLE_ID") ?? "1017447381127663717",
};
